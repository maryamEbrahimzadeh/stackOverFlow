		...
		public void run() {
			while (true) {
				try {
					...
					if (msg.getType() == Message.LOGIN) {
					...
					}else if (msg.getType() == Message.UPDATEPRO){
						if (!msg.getIslogin()) {
							mp.display.setText("can not update");
						}else{
							mp.display.setText(" update your profle");							
						}
						...
						if(msg.qadded()){							
							...
							mp.display.setText("");
							for(Document q : msg.getrelated() ){
								mp.display.append(q.toJson());
							}
						}
					...
					}else if(msg.getType()==Message.ANSWEREDIT){
						aep.setVisible(false);
						mp.setVisible(true);
					}else if (msg.getType()==Message.DELETEA){
						aep.setVisible(false);
						mp.setVisible(true);
						
					}else if(msg.getType()==Message.DELQUESTION){
						mp.dup.setVisible(false);
                        mp.setVisible(true);
					}else if(msg.getType()==Message.UPQUESTION){
						mp.dup.setVisible(false);
                        mp.setVisible(true);
					}
				} catch (IOException e) {
