...
public class Server {
	...
	private synchronized void answer(int type, Document info, ObjectOutputStream sOutput) {
		...
		if (type == Message.SUGNUP) {
			...
			MongoCollection<Document> collectiontquestion = database.getCollection("question");
			Vector<String> keywordsvec = (Vector<String>) info.get("tags");
			StringBuilder keywords = new StringBuilder();
			for (String w : keywordsvec) {
					keywords.append(w + " ");
			...
			boolean flag1=true;
			Vector<Document> qvec = new Vector<>();
			while (itrator.hasNext()) {
				Document q = itrator.next();
				boolean flag = true;
				ArrayList<String> tq = (ArrayList<String>) q.get("tags");
				for(String s : keywordsvec){
					if (!tq.contains(s)){
						flag = false;
					}
				}
				if(flag){// q mortabet ast
					flag1 = false;
					qvec.addElement(q);					
				}
			}
			...
			try {
				sOutput.writeObject(new Message(Message.ASK,true,qvec));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			try {
				sOutput.writeObject(new Message(Message.ASK,false,qvec));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private synchronized void answeraquistion(int type, Document answer, int qid, ObjectOutputStream sOutput) {
		if (type == Message.ANSWER) {
			...
			answer.append("aid", va.size()).append("opinions", new ArrayList<Document>()).append("rate", 0);
			va.add(answer);
	...
	private synchronized void RateAnswer(int type, Document q, Document ans, Document rate,
		...
		for (Document answer : va) {
			System.out.println(rate.getInteger("rate"));
			if (ans.getInteger("aid").equals(answer.getInteger("aid"))) {
				System.out.println(rate.getInteger("rate"));
				System.out.println(answer.getInteger("rate"));
	...
	public void answeredit(int type, Document q, int aid, Document updatedanswer, ObjectOutputStream sOutput) {
		...
		for (Document ans : va) {
			...
			if (ans.getInteger("aid").equals(aid)) {
				...
				a.add(updatedanswer);
				Document ansnew = new Document("aid", ans.getInteger("aid")).append("answer", ans.getString("answer"))
						.append("username", ans.getString("username")).append("updates", a);
				va2.add(ansnew);
	...
	private void updatepro(int type, Document info,Document user,  ObjectOutputStream sOutput) {
		MongoCollection<Document> collection = database.getCollection("USER");
		boolean flag = false;
		for (Document cur : collection.find()) {
		...
		if (!flag) {
			...
			Bson newValue = info;
			Bson updateOperationDocument = new Document("$set", newValue);
			collection.updateOne(user, updateOperationDocument);
			try {
				sOutput.writeObject(new Message(Message.UPDATEPRO, true, info));
			...
			try {
				sOutput.writeObject(new Message(Message.UPDATEPRO, false, info));
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	 ...
	 private synchronized void delQuestion(int type, Document users, Document q, ObjectOutputStream sOutput) {
	        MongoCollection<Document> collectiontquestion = database.getCollection("question");
	       
	        Document question = collectiontquestion.find(eq("qId", q.getInteger("qId"))).first();
	        
	        if (users.getString("username").equals(question.getString("questioner"))) {
	            collectiontquestion.deleteOne(eq("qId", question.get("qId")));
	        }
	    ...
	    private synchronized void upQuestion(int type, Document q, String s, ObjectOutputStream sOutput) {
	        MongoCollection<Document> collectiontquestion = database.getCollection("question");
	       
	        Document question = collectiontquestion.find(eq("qId", q.getInteger("qId"))).first();
	        
	        ArrayList<String> va = (ArrayList<String>) question.get("updates");
	        va.add(s);
	        ...
	        Document info = new Document("question", question.get("question"))
	                ...
	                .append("answers",question.get("answers") )
	                .append("updates",va );
	        collectiontquestion.insertOne(info);
	        try {
	            sOutput.writeObject(new Message(type));
		...
		public void run() {
			...
			while (keepGoing) {
				...
				case Message.DELETEA:
					...
					break;
				case Message.UPDATEPRO:
					updatepro(cm.getType(), cm.getqu(),cm.getUser(), sOutput);
					break;
				case Message.DELQUESTION:
                    delQuestion(cm.getType(), cm.getUser(), cm.getqu(), sOutput);
                    break;
                case Message.UPQUESTION:
                    upQuestion(cm.getType(), cm.getqu(), cm.getquestion(), sOutput);
                    break;
